...
const Header = ({ toggleSidebar }) => {
  ...
  return (
    <header className="d-flex align-items-center justify-content-between p-3 header-custom">
      ...
      />
      <h1 className="h5 mb-0 text-center school">SCHOOL MANAGEMENT SYSTEM</h1>
      <div className="d-flex align-items-center position-relative">
        ...
        >
          ...
          <FontAwesomeIcon icon={faBell} className="icon-circle" /></OverlayTrigger>
          {showNotifications && (
            <div className="dropdown-menu notification-dropdown">
              <div className="dropdown-item">
                <i className="bi bi-bell-slash"></i> No Notifications available
              </div>
            </div>
          )}
        </a>
        ...
        <a
          ...
          ref={profileRef}
        ><OverlayTrigger placement='bottom' overlay={<Tooltip>{userEmail}</Tooltip>}>
          {user && user.Picture ? (
            <img
          ...
          {showProfileMenu && (
            <div className="dropdown-menu profile-dropdown">
              ...
              <Link to="profile" className="dropdown-item" style={{ color: 'white' }}>
                <i className="bi bi-person-circle"></i> View Profile
              </Link>
              <button onClick={handleSignOut} className="dropdown-item" style={{ color: 'white' }}>
